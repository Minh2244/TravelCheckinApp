...
export default function BookingBatchPaymentScreen() {
  const params = useLocalSearchParams<SearchParams>();
  const router = useRouter();
  const insets = useSafeAreaInsets();

  const bookingIds = useMemo(() => {
    const raw = String(params.bookingIds || "").trim();
    if (!raw) return [];
    return raw
      .split(",")
      .map((x) => Number(x.trim()))
  ...
  useEffect(() => {
    if (bookingIds.length === 0) {
      setLoading(false);
      showToast("Thiếu danh sách phòng để thanh toán.");
      return;
    }
    ...
    bookingApi
      .createOrGetPaymentForBookingBatch(bookingIds)
      .then((response) => {
  ...
  async function handleConfirmTransfer() {
    if (!payment?.payment_id) {
      showToast("Thiếu mã thanh toán.");
      return;
    }

    setConfirming(true);
    try {
      await bookingApi.confirmRoomBatchTransfer(payment.payment_id);
      showToast("Đã xác nhận chuyển khoản gộp phòng thành công.");
      if (params.returnTo) {
  ...
  if (loading) {
    return (
      <View style={styles.loadingScreen}>
        ...
        <Text style={styles.loadingText}>Đang tải VietQR thanh toán gộp phòng...</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: Math.max(insets.top, 12) }]}>
        ...
        <View style={styles.headerText}>
          <Text style={styles.eyebrow}>VietQR Gộp</Text>
          <Text style={styles.title}>Thanh toán {bookingIds.length} phòng</Text>
        </View>
      </View>
      ...
      >
        <View style={styles.card}>
          <Text style={styles.paymentCode}>
            Payment #{payment?.payment_id || "-"} ({bookingIds.length} phòng)
          </Text>
          {isFreePayment ? (
        ...
        <View style={styles.noteCard}>
          ...
          <Text style={styles.noteText}>
            Bạn bấm xác nhận để hệ thống ghi nhận giao dịch thanh toán cho cả {bookingIds.length} phòng và chuyển tới Ví Thẻ Phòng.
          </Text>
        </View>
      </ScrollView>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 12) }]}>
        <Pressable
          ...
          onPress={isFreePayment ? () => router.replace("/wallet/room-pass") : handleConfirmTransfer}
          disabled={confirming}
        >
          <Text style={styles.primaryButtonText}>
            {isFreePayment ? "Xem vé phòng" : confirming ? "Đang xác nhận..." : "Xác nhận đã chuyển khoản"}
          </Text>
        </Pressable>
      </View>
    </View>

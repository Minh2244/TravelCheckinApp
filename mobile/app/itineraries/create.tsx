...
export default function CreateItineraryScreen() {
  ...
  const handleSave = async () => {
    if (!title.trim()) {
      Alert.alert("Lỗi", "Tên lịch trình không được để trống");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Thành công", "Đã tạo lịch trình", [
        ...
        <ScrollView style={styles.body} keyboardShouldPersistTaps="handled">
          ...
          {numDays > 0 && (
            <View style={styles.section}>
              ...
              {/* Items trong ngày */}
              {dayItems.length === 0 ? (
                <Text style={styles.emptyDay}>Chưa có địa điểm nào cho ngày {activeDay}</Text>
              ) : (
                dayItems.map((item, idx) => (
                  <View key={item.tempId} style={styles.itemCard}>
                    ...
                    <View style={styles.itemContent}>
                      ...
                      {item.custom_address ? (
                        <Text style={styles.itemAddress}>📍 {item.custom_address}</Text>
                      ) : null}
                      <View style={styles.itemMeta}>
                        ...
                        {item.estimated_cost ? (
                          <Text style={styles.itemMetaText}>💰 {Number(item.estimated_cost).toLocaleString("vi-VN")}đ</Text>
                        ) : null}
                      </View>
                    </View>
                    <TouchableOpacity onPress={() => removeItem(item.tempId)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
            ...
            <View style={styles.modalContent}>
              ...
              <ScrollView keyboardShouldPersistTaps="handled">
                ...
                <TextInput
                  ...
                  placeholder="Ví dụ: 123 Đường XYZ"
                />
                ...
                <View style={styles.row}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.label}>Chi phí (VND)</Text>
                    <TextInput
                      style={styles.input}
                      value={addCost}
                ...
                <TouchableOpacity
                  style={[styles.modalAddBtn, !customName.trim() && { opacity: 0.5 }]}
                  onPress={addCustom}
                  disabled={!customName.trim()}
                >
                  <Text style={styles.modalAddBtnText}>Thêm địa điểm tự do</Text>
                </TouchableOpacity>
              </ScrollView>
            </View>
          </View>

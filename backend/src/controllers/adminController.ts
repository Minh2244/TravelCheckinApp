...
export const getAdminVoucherStats = async (
  ...
  try {
    const [sysStats] = await pool.query<RowDataPacket[]>(
      `SELECT COUNT(*) as total,
    ...
    const [ownStats] = await pool.query<RowDataPacket[]>(
      `SELECT COUNT(*) as total,
              SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) as active_count,
              SUM(used_count) as total_uses
       FROM vouchers v
       JOIN users u ON v.owner_id = u.user_id
       WHERE u.role = 'owner' AND v.owner_deleted_at IS NULL`,
    );
    res.json({
      success: true,
      data: { system: sysStats[0] || {}, owner: ownStats[0] || {} },
    });
  } catch (error: any) {
...
export const createSystemVoucher = async (
  ...
  try {
    ...
    // Validate ngày bắt đầu >= hôm nay (so sánh theo ngày)
    ...
    const inferredScope = Array.isArray(location_ids)
      ...
      : location_id === null || location_id === "" || location_id === "all"
        ? "all"
        : "single";
    const validScopes = ["all", "single", "multiple", "owner_single", "owner_multiple"];
    const finalScope = validScopes.includes(rawScope) ? rawScope : inferredScope;
    ...
    if (finalScope === "owner_single") {
      const ownerIdNum = Number(owner_id);
      if (!Number.isFinite(ownerIdNum) || ownerIdNum <= 0) {
        res.status(400).json({ success: false, message: "Vui lòng chọn owner" });
        return;
      }
      const [ownerRows] = await pool.query<RowDataPacket[]>(
        `SELECT user_id FROM users WHERE user_id = ? AND role = 'owner' LIMIT 1`,
        [ownerIdNum],
      );
      if (!ownerRows[0]) {
        res.status(404).json({ success: false, message: "Owner không tồn tại" });
        return;
      }
      const specificLocId = parseId(location_id);
      if (specificLocId != null) {
        const [locRows] = await pool.query<RowDataPacket[]>(
          `SELECT location_id FROM locations WHERE location_id = ? AND owner_id = ? LIMIT 1`,
          [specificLocId, ownerIdNum],
        );
        if (!locRows[0]) {
          res.status(404).json({ success: false, message: "Địa điểm không thuộc owner này" });
          return;
        }
    ...
    if (finalScope === "owner_multiple") {
      const ownerIdsArr = Array.isArray(owner_ids)
        ? (owner_ids as unknown[])
            .map((x) => Number(x))
            .filter((n) => Number.isFinite(n) && n > 0)
        : [];
      if (ownerIdsArr.length === 0) {
        res.status(400).json({ success: false, message: "Vui lòng chọn ít nhất 1 owner" });
        return;
      }
      const placeholders = ownerIdsArr.map(() => "?").join(",");
      const [locRows] = await pool.query<RowDataPacket[]>(
        `SELECT location_id FROM locations WHERE owner_id IN (${placeholders})`,
        ownerIdsArr,
      );
      const ownerLocSet = new Set(
        (locRows as any[]).map((r) => Number(r.location_id)).filter(Number.isFinite),
      );
      const rawLocIds = Array.isArray(location_ids)
        ? (location_ids as unknown[])
            .map((x) => Number(x))
            .filter((n) => Number.isFinite(n))
        : [];
      if (rawLocIds.length > 0) {
        parsedLocationIds = rawLocIds.filter((id) => ownerLocSet.has(id));
      } else {
        parsedLocationIds = Array.from(ownerLocSet);
      }
      if (parsedLocationIds.length === 0) {
        res.status(400).json({ success: false, message: "Không có địa điểm nào hợp lệ" });
        return;
      }
    }
    ...
    if (["multiple", "owner_single", "owner_multiple"].includes(finalScope) && parsedLocationIds.length > 0) {
      const placeholders = parsedLocationIds.map(() => "?").join(",");
      const [locRows] = await pool.query<RowDataPacket[]>(
        `SELECT location_id FROM locations WHERE location_id IN (${placeholders})`,
        parsedLocationIds,
      );
    ...
    const [insertResult] = await pool.query<ResultSetHeader>(
      `INSERT INTO vouchers (
        ...
        start_date, end_date,
        usage_limit, max_uses_per_user, target_group, loyalty_min_spend,
        used_count, status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)`,
      [
        adminId,
...
export const updateOwnerVoucherAdmin = async (
  ...
  try {
    ...
    let desiredScope: "all" | "single" | "multiple" | "owner_single" | "owner_multiple" | null = null;
    ...
    if (wantsScopeUpdate) {
      ...
      if (
        ...
        rawScope === "single" ||
        rawScope === "multiple" ||
        rawScope === "owner_single" ||
        rawScope === "owner_multiple"
      ) {
      ...
      if (desiredScope === "owner_single") {
        const ownerIdNum = Number(body.owner_id ?? voucher.owner_id);
        const specificLocId = body.location_id != null && body.location_id !== "" ? Number(body.location_id) : null;
        if (specificLocId != null && Number.isFinite(specificLocId)) {
          const [locRows] = await pool.query<RowDataPacket[]>(
            `SELECT location_id FROM locations WHERE location_id = ? AND owner_id = ? LIMIT 1`,
            [specificLocId, ownerIdNum],
          );
          ...
          desiredLocationIds = [specificLocId];
        } else {
          const [locRows] = await pool.query<RowDataPacket[]>(
            `SELECT location_id FROM locations WHERE owner_id = ?`,
            [ownerIdNum],
          );
          desiredLocationIds = (locRows as any[]).map((r) => Number(r.location_id)).filter(Number.isFinite);
          if (desiredLocationIds.length === 0) {
            res.status(400).json({ success: false, message: "Owner không có địa điểm nào" });
            return;
          }
      ...
      if (desiredScope === "owner_multiple") {
        const ownerIdsArr = Array.isArray(body.owner_ids)
          ? (body.owner_ids as unknown[]).map(Number).filter((n) => Number.isFinite(n) && n > 0)
          : [];
        if (ownerIdsArr.length === 0) {
          res.status(400).json({ success: false, message: "Vui lòng chọn ít nhất 1 owner" });
          return;
        }
        const ph = ownerIdsArr.map(() => "?").join(",");
        const [locRows] = await pool.query<RowDataPacket[]>(
          `SELECT location_id FROM locations WHERE owner_id IN (${ph})`,
          ownerIdsArr,
        );
        const ownerLocSet = new Set((locRows as any[]).map((r) => Number(r.location_id)).filter(Number.isFinite));
        const rawLocIds = Array.isArray(body.location_ids)
          ? (body.location_ids as unknown[]).map(Number).filter(Number.isFinite)
          : [];
        if (rawLocIds.length > 0) {
          desiredLocationIds = rawLocIds.filter((id) => ownerLocSet.has(id));
        } else {
          desiredLocationIds = Array.from(ownerLocSet);
        }
        if (desiredLocationIds.length === 0) {
          res.status(400).json({ success: false, message: "Không có địa điểm nào hợp lệ" });
          return;

  ...
  try {
    ...
    for (const id of ids) {
      try {
        ...
        console.error(`❌ Booking publishToUser error for user ${id}:`, err?.message || err);
      }
    }
  } catch (err: any) {
    console.error("❌ notifyRealtimeBookingUpdate error:", err?.message || err);
  }
};
...
export const confirmTicketTransferHandler = async (
  ...
  try {
    ...
    void notifyRealtimeBookingUpdate(bookingId, "tourist_updated");
  } catch (error) {
    if (isBookingError(error)) {
      res
        .status(error.statusCode)
...
export const confirmTableTransferHandler = async (
  ...
  try {
    ...
    void notifyRealtimeBookingUpdate(bookingId, "pos_updated");
  } catch (error) {
    if (isBookingError(error)) {
      res
        .status(error.statusCode)
...
export const confirmRoomTransferHandler = async (
  ...
  try {
    ...
    void notifyRealtimeBookingUpdate(bookingId, "hotel_updated");
    void notifyRealtimeBookingUpdate(bookingId, "pms_updated");
  } catch (error) {
    if (isBookingError(error)) {
      res
        .status(error.statusCode)
...
export const cancelMyBookingHandler = async (
  ...
  try {
    ...
    const data = await cancelMyBooking({ userId, bookingId });
    res.status(200).json({
      success: true,
...
export const getMyTablePassHandler = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Lấy danh sách vỏ vé đặt bàn thành công",
      data,
    });
  } catch (error) {
    if (isBookingError(error)) {
    ...
    console.error("Lỗi getMyTablePassHandler:", error);
    res.status(500).json({
      success: false,
      message: "Lỗi server khi lấy vỏ vé đặt bàn",
    });
  }
};

export const getMyRoomPassHandler = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const locationIdRaw = Number((req.query as any)?.location_id);
    const data = await listMyRoomBookings({
      userId,
      locationId: Number.isFinite(locationIdRaw) ? locationIdRaw : null,
    });
    ...
    res.status(200).json({
      success: true,
      message: "Lấy danh sách vỏ vé đặt phòng thành công",
      data,
    });
  } catch (error) {
    if (isBookingError(error)) {
    ...
    console.error("Lỗi getMyRoomPassHandler:", error);
    res.status(500).json({
      success: false,
      message: "Lỗi server khi lấy vỏ vé đặt phòng",
    });
  }
};

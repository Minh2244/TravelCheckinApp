...
const publishLocationEvent = async (
  ...
  try {
    console.log(`[SSE] Broadcasting event to all connected clients:`, JSON.stringify(event));
    publishToAll(event);
  } catch (err: any) {
    console.error("❌ publishToAll error:", err?.message || err);
  }
  ...
  try {
    ...
    if (userIds && userIds.length > 0) {
      console.log(`[SSE] Publishing event to users ${userIds.join(",")}:`, JSON.stringify(event));
      publishToUsers(userIds, event);
    }
  } catch (err: any) {
    console.error("❌ getRealtimeRecipientsForLocation/publishToUsers error:", err?.message || err);
  }
};
...
export const getPosPaymentsHistory = async (
  ...
  try {
    ...
    const where: string[] = [
      "p.location_id = ?",
      "p.status = 'completed'",
      // POS food invoices OR hotel stay invoices OR tourist tickets (offline/online)
      "(p.notes LIKE '%\"service_type\":\"food\"%' OR p.notes LIKE '%\"service_type\":\"table\"%' OR p.notes LIKE 'HOTEL_STAY:%' OR p.notes LIKE 'HOTEL_STAYS:%' OR p.notes LIKE 'TOURIST_TICKETS:%' OR p.notes LIKE '%\"service_type\":\"ticket\"%')",
    ];
    ...
    // Truy vấn phụ để lấy toàn bộ các payment online đã hoàn thành nhằm làm giàu dữ liệu trả trước (hỗ trợ cả các giao dịch cũ)
    const [allPrepaids] = await pool.query<RowDataPacket[]>(
      `SELECT booking_id, amount, notes
       FROM payments
       WHERE location_id = ?
    ...
    const prepaidMap = new Map<number, { amount: number; items: any[] }>();
    for (const p of allPrepaids) {
      const bid = Number((p as any).booking_id);
      if (!Number.isFinite(bid)) continue;
      const parsed = parsePaymentNotesJson((p as any).notes);
      const parsedItems = Array.isArray(parsed?.items) ? parsed.items : [];
      if (parsed?.invoice_ready === true && parsedItems.length > 0) {
        prepaidMap.set(bid, {
    ...
    const byBucket = new Map<
      string,
      { total: number; booking_base: number; commission: number; after_commission: number }
    >();
    ...
    // Identify booking IDs that have a counter (food) payment to hide duplicate prepaid online booking entries
    const bookingIdsWithFood = new Set<number>();
    for (const r of rows) {
      const notes = parsePaymentNotesJson(r.notes);
      const isFood = Boolean(notes) && String(notes.service_type || "") === "food";
      const bookingId = (r as any).booking_id == null ? null : Number((r as any).booking_id);
      if (isFood && bookingId && bookingId > 0) {
        bookingIdsWithFood.add(bookingId);
      }
    }
    ...
    const [allLocPayments] = await pool.query<RowDataPacket[]>(
      `SELECT p.payment_id, p.booking_id, p.notes, p.transaction_source,
       ...
       WHERE p.location_id = ?
         ...
         AND (p.notes LIKE '%"service_type":"food"%' OR p.notes LIKE '%"service_type":"table"%' OR p.notes LIKE 'HOTEL_STAY:%' OR p.notes LIKE 'HOTEL_STAYS:%' OR p.notes LIKE 'TOURIST_TICKETS:%' OR p.notes LIKE '%"service_type":"ticket"%')
       ORDER BY p.payment_time ASC, p.payment_id ASC`,
      [locationId]
    );

    const bookingIdsWithFoodAll = new Set<number>();
    for (const pRow of allLocPayments) {
      const pNotes = parsePaymentNotesJson(pRow.notes);
      const isFood = Boolean(pNotes) && String(pNotes.service_type || "") === "food";
      const bookingId = pRow.booking_id == null ? null : Number(pRow.booking_id);
      if (isFood && bookingId && bookingId > 0) {
        bookingIdsWithFoodAll.add(bookingId);
      }
    }
    ...
    for (const pRow of allLocPayments) {
      const pNotes = parsePaymentNotesJson(pRow.notes);
      const isTable = Boolean(pNotes) && String(pNotes.service_type || "") === "table";
      const isFood = Boolean(pNotes) && String(pNotes.service_type || "") === "food";
      const notesStr = String(pRow.notes || "");
      const isHotel = notesStr.startsWith("HOTEL_STAY:") || notesStr.startsWith("HOTEL_STAYS:");
      const isTicketOnline = Boolean(pNotes) && String(pNotes.service_type || "") === "ticket";
      const isTourist = notesStr.startsWith("TOURIST_TICKETS:") || isTicketOnline;

      if (!isFood && !isTable && !isHotel && !isTourist) continue;

      const bookingId = pRow.booking_id == null ? null : Number(pRow.booking_id);
      const transactionSource = pRow.transaction_source ? String(pRow.transaction_source) : "";

      if (isTable && bookingId && bookingIdsWithFoodAll.has(bookingId)) {
        continue;
      }

      const bookingStatus = pRow.booking_status ? String(pRow.booking_status).toLowerCase() : "";
      if (
        isTable &&
        transactionSource === "online_booking" &&
        pNotes?.invoice_ready !== true &&
      ...
      seqCounter++;
      seqMap.set(Number(pRow.payment_id), seqCounter);
    }
    ...
    for (const r of rows) {
      ...
      const isTicketOnline =
        Boolean(notes) && String(notes.service_type || "") === "ticket";
      const isTourist = notesStr.startsWith("TOURIST_TICKETS:") || isTicketOnline;
      if (!isFood && !isTable && !isHotel && !isTourist) continue;

      const amount = Number(r.amount || 0);
      const voucherCode = (r as any).booking_voucher_code || notes?.voucher_code || null;
      const discountAmount = Number((r as any).booking_discount_amount || notes?.discount_amount || 0);
      const finalAmountVal = Number((r as any).booking_final_amount || notes?.final_amount || amount);
      ...
      if (isTable && bookingId && bookingIdsWithFood.has(bookingId)) {
        // Skip online table preorder entry because its payment has been merged into the counter food invoice
        continue;
      }
      ...
      const itemsRaw =
        (isFood || isTable) && notes && Array.isArray(notes.items)
          ...
          : isTourist &&
            ...
            : [];
      const bookingStatus = (r as any).booking_status ? String((r as any).booking_status).toLowerCase() : "";
      if (
        isTable &&
        transactionSource === "online_booking" &&
      ...
      let prepaidVal = 0;
      let prepaidItemsVal: any[] = [];
      if (bookingId && (isFood || isTable) && notes?.invoice_ready !== true) {
        const prep = prepaidMap.get(bookingId);
        if (prep) {
          prepaidVal = prep.amount;
          prepaidItemsVal = prep.items;
        } else if (notes && Array.isArray(notes.prepaid_items)) {
          prepaidVal = Number(notes.prepaid_amount || 0);
          prepaidItemsVal = notes.prepaid_items;
        }
      }

      let onlineTicketQty: number | null = null;
      if (isTicketOnline && qrParsed && Array.isArray(qrParsed.ticket_items)) {
        onlineTicketQty = qrParsed.ticket_items.reduce((s: number, x: any) => s + Number(x.quantity || 0), 0);
      }

      let totalQty = 0;
      if (isFood || isTable) {
        const onsiteQty = items.reduce((s: number, x: any) => s + Number(x.quantity || 0), 0);
        const prepaidQty = prepaidItemsVal.reduce((s: number, x: any) => s + Number(x.quantity || 0), 0);
        totalQty = onsiteQty + prepaidQty;
      } else {
        totalQty = Number(
      ...
      // prepaidVal and prepaidItemsVal are calculated earlier in the loop to support totalQty calculation
      ...
      let paymentContribution = amount;
      if (isHotel && hotelPrepaidAmount > 0) {
        paymentContribution += hotelPrepaidAmount;
      } else if ((isFood || isTable) && prepaidVal > 0) {
        paymentContribution += prepaidVal;
      }

      const dbCommission = r.commission_amount != null ? Number(r.commission_amount) : null;
      let commissionAmount = dbCommission !== null
        ? dbCommission
        : (isPrebookedUser
          ? +Number((paymentContribution * commissionRate) / 100).toFixed(2)
          : 0);

      if (((isFood || isTable) && prepaidVal > 0 || isHotel && hotelPrepaidAmount > 0) && commissionRate > 0) {
        commissionAmount = +Number((paymentContribution * commissionRate) / 100).toFixed(2);
      }

      const dbReceivable = r.owner_receivable != null ? Number(r.owner_receivable) : null;
      let ownerReceivable = dbReceivable !== null
        ? dbReceivable
        : (paymentContribution - commissionAmount);

      if (((isFood || isTable) && prepaidVal > 0) || (isHotel && hotelPrepaidAmount > 0)) {
        ownerReceivable = paymentContribution - commissionAmount;
      }
      ...
      history.push({
        ...
        invoice_no: seqMap.get(paymentId) || 1,
        pos_order_id: isFood || isTable ? Number(notes?.pos_order_id || 0) || null : null,
      ...
      total += paymentContribution;

      const isCashPayment = (method: string) => {
        const m = String(method || "").trim().toLowerCase();
        return m === "cash" || m.includes("tien mat") || m.includes("tiền mặt");
      };

      if (isHotel && hotelPrepaidAmount > 0) {
        if (isCashPayment(pm)) totalCash += amount;
        else totalTransfer += amount;

        const isPrepaidCash = isCashPayment(hotelPrepaidPaymentMethod || "");
        if (isPrepaidCash) {
          totalCash += hotelPrepaidAmount;
        } else {
          totalTransfer += hotelPrepaidAmount;
        }
      } else {
        if (isCashPayment(pm)) totalCash += amount;
        else totalTransfer += amount;

        if ((isFood || isTable) && prepaidVal > 0) {
          totalTransfer += prepaidVal;
        }
      }

      if (isPrebookedUser) bookingBaseTotal += paymentContribution;
      ...
      const prev = byBucket.get(bucketKey) || { total: 0, booking_base: 0, commission: 0, after_commission: 0 };
      byBucket.set(bucketKey, {
        total: prev.total + paymentContribution,
        booking_base: prev.booking_base + (isPrebookedUser ? paymentContribution : 0),
        commission: prev.commission + commissionAmount,
        after_commission: prev.after_commission + ownerReceivable,
      });
    }
    ...
    const series = bucketKeys.map((bucketKey) => {
      const v = byBucket.get(bucketKey) || { total: 0, booking_base: 0, commission: 0, after_commission: 0 };
      const commission = Number(v?.commission || 0);
      const totalBucket = Number(v?.total || 0);
      const afterCommission = Number(v?.after_commission || totalBucket - commission);
      return {
        day: bucketKey,
    ...
    const displayHistory = history.reduce<typeof history>((acc, row) => {
      // Gộp mọi booking có nhiều payment (trừ hotel vì hotel có prepaid+onsite riêng)
      const canMerge =
        row.booking_id != null && !row.hotel;
      ...
      existing.commission_amount = toMoney2(
        Number(existing.commission_amount || 0) + Number(row.commission_amount || 0),
      );
      existing.total_qty =
    ...
    // Đánh lại số hóa đơn sau khi merge để mỗi row có số duy nhất
    let invCounter = 0;
    for (const row of displayHistory) {
      invCounter++;
      row.invoice_no = invCounter;
    }

    const commissionAmountTotal = history.reduce((sum, h) => sum + Number(h.commission_amount || 0), 0);
...
export const checkinSecureQr = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const auth = await getAuth(req);
    const { location_id, qr_data } = req.body as {
      location_id: number;
      qr_data: string;
    };
    ...
    if (!Number.isFinite(locationId) || locationId <= 0) {
      res.status(400).json({ success: false, message: "location_id không hợp lệ" });
      return;
    }

    if (!qr_data || !String(qr_data).trim()) {
      res.status(400).json({ success: false, message: "Dữ liệu QR hoặc mã vé không được trống" });
      return;
    }
    ...
    const cleanData = String(qr_data).trim();

    // 1) Try parsing as JSON secure QR
    try {
      if (cleanData.startsWith("{") && cleanData.endsWith("}")) {
        const verified = verifySecureQrPayload(cleanData, locationId);
        bookingId = Number(verified.bookingId);
      }
    } catch (err: any) {
      res.status(400).json({ success: false, message: err?.message || "Mã QR bảo mật đã bị thay đổi hoặc giả mạo!" });
      return;
    }
    ...
    if (!bookingId) {
      ...
      if (["BK", "DI", "RS", "SB", "BT"].includes(parts[0])) {
        if (parts.length === 2) {
    ...
    if (!bookingId) {
      res.status(400).json({
        success: false,
        message: "Mã QR hoặc mã vé nhập thủ công không hợp lệ hoặc sai định dạng bảo mật!",
      });
      return;
    }
    ...
    try {
      ...
      // Look up booking and ensure correct location
      const [bRows] = await conn.query<RowDataPacket[]>(
        `SELECT b.booking_id, b.user_id, b.location_id, b.status AS booking_status,
                b.contact_name, b.contact_phone, b.pos_order_id, b.created_at,
                s.service_type, b.service_id, s.service_name, b.check_in_date, b.check_out_date
         FROM bookings b
         JOIN services s ON s.service_id = b.service_id
         WHERE b.booking_id = ?
         LIMIT 1
      ...
      if (!booking) {
        await conn.rollback();
        res.status(404).json({ success: false, message: "Không tìm thấy thông tin vé đặt này!" });
        return;
      }

      const preorderOrderIdRaw = booking.pos_order_id;
      const preorderOrderId =
        preorderOrderIdRaw == null ? null : Number(preorderOrderIdRaw);

      if (Number(booking.location_id) !== locationId) {
        await conn.rollback();
        res.status(400).json({
          success: false,
          message: "Mã đặt chỗ này thuộc về địa điểm khác. Vui lòng quét đúng nơi!",
        });
        return;
      }
      ...
      if (bookingStatus === "cancelled") {
        await conn.rollback();
        res.status(400).json({ success: false, message: "Vé này đã bị hủy trên hệ thống!" });
        return;
      }

      if (bookingStatus === "completed") {
        await conn.rollback();
        res.status(400).json({ success: false, message: "Vé đặt này đã được sử dụng check-in trước đó!" });
        return;
      }

      // ROOM BOOKING BRANCH
      if (String(booking.service_type).toLowerCase() === "room") {
        // If pending, first confirm it
        if (bookingStatus === "pending") {
          const createdAt = booking.created_at ? new Date(booking.created_at) : null;
          const now30MinLater = createdAt ? new Date(createdAt.getTime() + 30 * 60 * 1000) : null;
          const nowMs = Date.now();
          if (!createdAt || !now30MinLater || nowMs < now30MinLater.getTime()) {
            const minutesLeft = createdAt
              ? Math.max(1, Math.ceil((now30MinLater!.getTime() - nowMs) / 60000))
              : 30;
            await conn.rollback();
            res.status(400).json({
              success: false,
              message: `Vé đặt phòng này chưa được chủ cơ sở xác nhận. Vui lòng đợi khoảng ${minutesLeft} phút nữa để hệ thống tự xác nhận, hoặc nhờ chủ cơ sở duyệt trực tiếp từ tab "Chờ duyệt".`,
            });
            return;
          }

          // Auto-confirm booking inline
          await conn.query(
        ...
        const [stayRows] = await conn.query<RowDataPacket[]>(
          `SELECT stay_id, room_id, expected_checkin, expected_checkout, notes, user_id
           FROM hotel_stays
        ...
        const roomId = Number(stay.room_id);
        const stayId = Number(stay.stay_id);
        const userId = stay.user_id == null ? Number(booking.user_id) : Number(stay.user_id);

        const [rRows] = await conn.query<RowDataPacket[]>(
          `SELECT room_number FROM hotel_rooms WHERE room_id = ? LIMIT 1`,
          [roomId]
        );
        const roomNumber = String(rRows[0]?.room_number || "").trim() || `Phòng ${roomId}`;

        let contactName = booking.contact_name;
        let contactPhone = booking.contact_phone;
        if (!contactName || !contactName.trim() || contactName === "Ẩn danh" || !contactPhone || !contactPhone.trim() || contactPhone === "-") {
          const [uRows] = await conn.query<RowDataPacket[]>(
            `SELECT full_name, phone FROM users WHERE user_id = ? LIMIT 1`,
            [userId]
          );
          if (uRows[0]) {
            if (!contactName || !contactName.trim() || contactName === "Ẩn danh") {
              contactName = uRows[0].full_name;
            }
            if (!contactPhone || !contactPhone.trim() || contactPhone === "-") {
              contactPhone = uRows[0].phone;
            }
          }
        }
        ...
        const now = new Date();
        const expectedCheckinDate = stay.expected_checkin ? new Date(String(stay.expected_checkin)) : now;
        const expectedCheckoutDate = stay.expected_checkout ? new Date(String(stay.expected_checkout)) : now;

        // Preserve planned stay duration
        let plannedDurationMs = 24 * 60 * 60 * 1000;
        if (expectedCheckinDate && expectedCheckoutDate) {
          const diff = expectedCheckoutDate.getTime() - expectedCheckinDate.getTime();
          if (diff > 0) plannedDurationMs = diff;
        }
        const expectedCheckoutByRoom = new Date(now.getTime() + plannedDurationMs);

        // Stay notes
        const stayNotes = {
          guest_full_name: contactName || null,
          guest_phone: contactPhone || null,
          stay_days: Math.max(1, Math.ceil(plannedDurationMs / (24 * 60 * 60 * 1000))),
          stay_nights: Math.max(1, Math.ceil(plannedDurationMs / (24 * 60 * 60 * 1000))),
          source: "prebook_qr",
        };

        const qrText = `Cập nhật check-in bảo mật cho đơn #BK-${bookingId}`;

        // 1) Mark check-in record verified
        const [checkinInsert] = await conn.query<ResultSetHeader>(
          `INSERT INTO checkins (user_id, location_id, booking_id, status, verified_by, notes)
           VALUES (?, ?, ?, 'verified', ?, ?)`,
          [userId, locationId, bookingId, auth.userId, qrText]
        );

        // 2) Convert stay status to inhouse
        await conn.query(
          `UPDATE hotel_stays
           SET status = 'inhouse',
               checkin_time = ?,
        ...
        res.json({
          success: true,
          message: "Check-in bảo mật phòng thành công!",
          data: {
            contactName: contactName,
            contactPhone: contactPhone,
            roomNames: [roomNumber],
            secureCode: cleanData.startsWith("{") ? "QR bộc bảo mật" : cleanData,
          },
        });
        return;
      }

      // Kiểm tra: nếu booking còn pending, chỉ cho check-in nếu đã qua 30 phút (auto-confirm window)
      if (bookingStatus === "pending") {
        const createdAt = booking.created_at ? new Date(booking.created_at) : null;
        const now30MinLater = createdAt ? new Date(createdAt.getTime() + 30 * 60 * 1000) : null;
        const nowMs = Date.now();
        if (!createdAt || !now30MinLater || nowMs < now30MinLater.getTime()) {
          ...
          res.status(400).json({
            success: false,
            message: `Vé đặt bàn này chưa được chủ cơ sở xác nhận. Vui lòng đợi khoảng ${minutesLeft} phút nữa để hệ thống tự xác nhận, hoặc nhờ chủ cơ sở duyệt trực tiếp từ tab "Chờ duyệt".`,
          });
          return;
        }
      }

      // Find reserved tables for this booking
      const [rRows] = await conn.query<RowDataPacket[]>(
        `SELECT r.table_id, t.table_name, r.status AS reservation_status
         FROM booking_table_reservations r
         JOIN pos_tables t ON t.table_id = r.table_id
         WHERE r.booking_id = ?
         FOR UPDATE`,
      ...
      const bookingTableIds = rRows.map((r) => Number(r.table_id));
      const tableNames = rRows.map((r) => String(r.table_name || "").trim()).filter(Boolean);

      if (bookingTableIds.length > 0) {
        const placeholders = bookingTableIds.map(() => "?").join(",");
        // 1) Mark tables occupied
        await conn.query(
          `UPDATE pos_tables
           SET status = 'occupied'
           WHERE location_id = ?
        ...
        // Hủy toàn bộ đơn 'open' rác cũ trên các bàn này ngoại trừ preorderOrderId
        const cancelParams = [locationId, ...bookingTableIds];
        let cancelQuery = `UPDATE pos_orders
                           SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP
                           WHERE location_id = ?
                             AND table_id IN (${placeholders})
                             AND status = 'open'`;
        if (preorderOrderId) {
          cancelQuery += ` AND order_id <> ?`;
          cancelParams.push(preorderOrderId);
        }
        await conn.query(cancelQuery, cancelParams);

        // 2) Mark reservations checked_in
        await conn.query(
      ...
      if (preorderOrderId && Number.isFinite(preorderOrderId)) {
        ...
        if (paymentRow?.payment_id) {
          ...
          const updatedNotes = {
            ...
            table_name: tableNames.length > 0 ? tableNames.join(", ") : null,
            table_names: tableNames.length > 0 ? tableNames.join(", ") : null,
            items: preorderItems,
      ...
      // 3) Create a verified checkin record
      const bookingUserId = booking.user_id == null ? null : Number(booking.user_id);
      ...
      // Tự động tạo đơn extras 'open' mới trị giá 0đ cho bàn chính của booking
      let primaryTableIdForExtras: number = bookingTableIds[0] || 0;
      if (preorderOrderId && Number.isFinite(preorderOrderId)) {
        const [oRows] = await conn.query<RowDataPacket[]>(
          `SELECT order_id, table_id FROM pos_orders WHERE order_id = ? LIMIT 1 FOR UPDATE`,
          [preorderOrderId],
        );
        if (oRows[0]?.table_id) {
          primaryTableIdForExtras = Number(oRows[0].table_id);
        }
      }

      if (primaryTableIdForExtras > 0) {
        ...
        let extrasOrderId: number | null = null;
        if (hasOrderSourceColumn) {
          ...
          if (openOrders[0]) {
            extrasOrderId = Number(openOrders[0].order_id);
          }
        }
      ...
      res.json({
        ...
        data: {
          ...
          contactName: contactName,
          contactPhone: contactPhone,
          tableNames: tableNames.length > 0 ? tableNames : ["Bàn ăn uống"],
          secureCode: cleanData.startsWith("{") ? "QR bộc bảo mật" : cleanData,
        },
      });
    } catch (e) {
...
export const getOwnerVouchers = async (
  ...
  try {
    ...
    // Thêm voucher admin tạo áp dụng cho địa điểm của owner
    let adminRows: RowDataPacket[] = [];
    try {
      const [rows2] = await pool.query<RowDataPacket[]>(
        `SELECT v.voucher_id, v.code, v.campaign_name, v.campaign_description,
                v.discount_type, v.discount_value, v.min_order_value, v.max_discount_amount,
                v.start_date, v.end_date, v.usage_limit, v.used_count, v.status,
                v.apply_to_service_type, v.location_id,
                COALESCE(
                  GROUP_CONCAT(loc.location_name SEPARATOR ', '),
                  (SELECT l3.location_name FROM locations l3 WHERE l3.location_id = v.location_id LIMIT 1)
                ) as location_name,
                CASE WHEN v.end_date < NOW() THEN 'expired' ELSE v.status END as computed_status,
                'admin' as voucher_source
         FROM vouchers v
         JOIN users u ON u.user_id = v.owner_id
         LEFT JOIN voucher_locations vl ON vl.voucher_id = v.voucher_id
         LEFT JOIN locations loc ON loc.location_id = vl.location_id AND loc.owner_id = ?
         WHERE u.role = 'admin'
           AND v.owner_deleted_at IS NULL
           AND v.status = 'active'
           AND (
      ...
      // Fallback nếu bảng voucher_locations không tồn tại
      if (e?.code === "ER_NO_SUCH_TABLE" && String(e?.message || "").includes("voucher_locations")) {
        const [rows2] = await pool.query<RowDataPacket[]>(
          `SELECT v.voucher_id, v.code, v.campaign_name, v.campaign_description,
                  v.discount_type, v.discount_value, v.min_order_value, v.max_discount_amount,
                  v.start_date, v.end_date, v.usage_limit, v.used_count, v.status,
    ...
    res.json({ success: true, data: rows, adminVouchers: adminRows });
  } catch (error: any) {
    res
      .status(error?.statusCode || 500)
      .json({ success: false, message: error?.message || "Lỗi server" });
...
export const createOwnerVoucher = async (
  ...
  try {
    ...
    // Chấp nhận cả location_id hoặc location_ids
    const hasLocationId = "location_id" in body;
    const hasLocationIds = Array.isArray(body.location_ids) && body.location_ids.length > 0;
    if (!hasLocationId && !hasLocationIds) {
      res.status(400).json({
        success: false,
    ...
    const rawLocationIds = body.location_ids;
    const hasMultiple = Array.isArray(rawLocationIds) && rawLocationIds.length > 0;

    let locationId: number | null = null;
    let parsedLocationIds: number[] = [];

    if (hasMultiple) {
      parsedLocationIds = rawLocationIds
        .map((v: any) => Number(v))
        .filter((n: number) => Number.isFinite(n) && n > 0);
      if (parsedLocationIds.length === 0) {
        res.status(400).json({ success: false, message: "Danh sách địa điểm không hợp lệ" });
        return;
      }
      // Verify all locations belong to owner
      const placeholders = parsedLocationIds.map(() => "?").join(",");
      const [locRows] = await pool.query<RowDataPacket[]>(
        `SELECT location_id FROM locations WHERE location_id IN (${placeholders}) AND owner_id = ?`,
        [...parsedLocationIds, auth.userId],
      );
      if (locRows.length !== parsedLocationIds.length) {
        res.status(403).json({ success: false, message: "Một hoặc nhiều địa điểm không thuộc owner" });
        return;
      }
      locationId = null; // multiple → location_id = NULL, dùng voucher_locations
    } else {
      locationId =
      ...
      if (locationId != null) {
        ...
        if (!locRows[0]) {
          res.status(403).json({ success: false, message: "Địa điểm không thuộc owner" });
          return;
        }
      }
    }

    // Validate ngày (so sánh theo ngày, bỏ qua giờ phút)
    ...
    if (startDate < today) {
      res.status(400).json({ success: false, message: "Ngày bắt đầu phải từ hôm nay trở đi" });
      return;
    }
    ...
    const [dup] = await pool.query<RowDataPacket[]>(
      `SELECT voucher_id FROM vouchers WHERE code = ? AND owner_id = ? AND owner_deleted_at IS NULL`,
      [code, auth.userId],
    );
    if (dup[0]) {
      res.status(400).json({ success: false, message: "Mã voucher đã tồn tại" });
      return;
    }
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO vouchers (
        ...
        usage_limit, max_uses_per_user, target_group, loyalty_min_spend, status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')`,
      [
        auth.userId,
        locationId,
        ...
        body.target_group ?? "all",
        body.loyalty_min_spend ?? null,
      ],
    );

    const voucherId = result.insertId;

    // Insert voucher_locations nếu chọn nhiều địa điểm
    if (parsedLocationIds.length > 0) {
      try {
...
export const updateOwnerVoucher = async (
  ...
  try {
    ...
    // Validate ngày nếu sửa start_date (so sánh theo ngày)
    if (body.start_date) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const startDate = new Date(body.start_date);
    ...
    up("target_group", body.target_group);
    up("loyalty_min_spend", body.loyalty_min_spend);
    ...
    // Handle location_ids update
    if (Array.isArray(body.location_ids)) {
      const parsedIds = body.location_ids
        .map((v: any) => Number(v))
        .filter((n: number) => Number.isFinite(n) && n > 0);

      // Xóa voucher_locations cũ
      try {
        await pool.query(`DELETE FROM voucher_locations WHERE voucher_id = ?`, [voucherId]);
      } catch (e: any) {
        if (!(e?.code === "ER_NO_SUCH_TABLE" && String(e?.message || "").includes("voucher_locations"))) throw e;
      }

      // Thêm voucher_locations mới
      if (parsedIds.length > 0) {
        try {
          const valuesSql = parsedIds.map(() => "(?, ?)").join(", ");
          const flatParams = parsedIds.flatMap((id: number) => [voucherId, id]);
          await pool.query(
            `INSERT INTO voucher_locations (voucher_id, location_id) VALUES ${valuesSql}`,
            flatParams,
          );
...
export const getOwnerVoucherStats = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const [statsRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
       ...
       FROM vouchers WHERE owner_id = ? AND owner_deleted_at IS NULL`,
      [auth.userId],
    );

    const [topRows] = await pool.query<RowDataPacket[]>(
      `SELECT v.voucher_id, v.campaign_name, v.used_count,
              COALESCE(SUM(vuh.discount_amount), 0) as total_discount
       FROM vouchers v
       LEFT JOIN voucher_usage_history vuh ON vuh.voucher_id = v.voucher_id
       WHERE v.owner_id = ? AND v.owner_deleted_at IS NULL
       GROUP BY v.voucher_id
    ...
    res.json({
      success: true,
      data: { summary: statsRows[0] || {}, topVouchers: topRows || [] },
    });
  } catch (error: any) {
    res
      .status(error?.statusCode || 500)
...
export const getHotelRooms = async (
  ...
  try {
    ...
    const dataRows = rows.map((row) => {
      ...
      let stay_id = row.stay_id;
      let expected_checkin = row.expected_checkin;
      let expected_checkout = row.expected_checkout;
...
export const setHotelRoomStatus = async (
  ...
  try {
    ...
    if (nextStatus === "vacant") {
      const [activeStays] = await pool.query<RowDataPacket[]>(
        `SELECT status FROM hotel_stays WHERE room_id = ? AND status IN ('reserved', 'inhouse') LIMIT 1`,
        [roomId]
      );
      if (activeStays[0]) {
        const stayStatus = String(activeStays[0].status);
        if (stayStatus === "reserved") {
          finalStatus = "reserved";
        } else if (stayStatus === "inhouse") {
  ...
  try {
    ...
    try {
      ...
      const expectedIsValid =
        currentExpected != null && Number.isFinite(currentExpected.getTime()); const base = expectedIsValid ? currentExpected! : now;
...
export const checkoutHotelStay = async (
  ...
  try {
    ...
    try {
      ...
      }      // 1. Get default commission and vat rates from system settings
      const [settingsRows] = await conn.query<RowDataPacket[]>(
        `SELECT setting_key, setting_value FROM system_settings
         WHERE setting_key IN ('default_commission_rate','vat_rate')`,
      );
      ...
      // 2. Get location commission rate
      const [locRateRows] = await conn.query<RowDataPacket[]>(
        `SELECT commission_rate FROM locations WHERE location_id = ? LIMIT 1`,
        [locationId],
      );
      ...
      const prepaidAmount = bookingId != null ? prepaidCandidate : 0;
      const onsiteSubtotal = toMoney(finalSubtotal - prepaidAmount);
      ...
      if (flowStep === "init") {
        ...
        if (pendingRows[0]?.payment_id) {
          ...
          if (support.hasTransactionSource) {
            ...
            setParams.push(bookingId != null ? "online_booking" : "onsite_pos");
          }
          if (support.hasPerformedByUserId) {
          ...
          if (support.hasTransactionSource) {
            columns.push("transaction_source");
            values.push("?");
            params.push(bookingId != null ? "online_booking" : "onsite_pos");
          }
          if (support.hasPerformedByUserId) {
            columns.push("performed_by_user_id");
            values.push("?");
      ...
      invoiceSnapshot.payment_id = paymentId; await conn.query(
        `UPDATE hotel_stays
         SET subtotal_amount = ?, discount_amount = 0.00, final_amount = ?,
             status = 'checked_out', checkout_time = ?, closed_by = ?
         WHERE stay_id = ?`,
        [finalSubtotal, finalAmount, now, auth.userId, stayId],
      );
  ...
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT
         ...
         rb_next.check_in_date AS upcoming_check_in_date,
         rb_next.contact_name AS upcoming_contact_name,
         rb_next.contact_phone AS upcoming_contact_phone
       FROM pos_tables t
       LEFT JOIN (
         SELECT r.table_id,
                MAX(r.reservation_id) AS reservation_id,
                SUBSTRING_INDEX(
                  GROUP_CONCAT(r.status ORDER BY r.start_time DESC, r.reservation_id DESC),
                  ',',
                  1
         ...
         WHERE r.location_id = ?
           ...
           AND (
             r.status = 'checked_in'
             OR NOW() BETWEEN DATE_SUB(r.start_time, INTERVAL 1 HOUR) AND DATE_ADD(r.start_time, INTERVAL 4 HOUR)
           )
         GROUP BY r.table_id
       ) r_current
       ...
       LEFT JOIN booking_table_reservations r_next
         ON r_next.reservation_id = (
       ...
       LEFT JOIN bookings rb_next
         ON rb_next.booking_id = r_next.booking_id
  ...
  try {
    ...
    try {
      ...
      const [resRows] = await conn.query<RowDataPacket[]>(
        `SELECT r.reservation_id FROM booking_table_reservations r
         JOIN bookings b ON b.booking_id = r.booking_id
         WHERE r.table_id = ?
           AND r.status IN ('active', 'checked_in')
           AND r.actual_end_time IS NULL
      ...
      const isOnlineBooking = resRows.length > 0 || tableStatus === "reserved";
      const orderSource: "online_booking" | "onsite_pos" =
        isOnlineBooking ? "online_booking" : "onsite_pos";
  ...
  try {
    ...
    try {
      ...
      if (status === "reserved") {
        ...
        res.status(403).json({
          success: false,
          message: "Bàn đã đặt trước. Vui lòng quét mã QR check-in trước khi mở bàn.",
        });
        return;
      }
      ...
      const [bRows] = await conn.query<RowDataPacket[]>(
        `SELECT b.booking_id, b.user_id, b.pos_order_id
         ...
         WHERE r.table_id = ?
           ...
           AND r.status = 'active'
            AND r.actual_end_time IS NULL
            AND NOW() BETWEEN DATE_SUB(r.start_time, INTERVAL 1 HOUR) AND DATE_ADD(r.start_time, INTERVAL 4 HOUR)
      ...
      let orderSource: "online_booking" | "onsite_pos" = "onsite_pos";
      if (status === "reserved") {
        orderSource = "online_booking";
      } else if (status === "occupied") {
        const [checkedInReservation] = await conn.query<RowDataPacket[]>(
          `SELECT r.booking_id
           FROM booking_table_reservations r
           WHERE r.table_id = ?
...
export const addPosOrderItem = async (
  ...
  try {
    ...
    const [linkedBookings] = await pool.query<RowDataPacket[]>(
      `SELECT booking_id FROM bookings WHERE pos_order_id = ? LIMIT 1`,
      [orderId],
    );
    if (linkedBookings[0]) {
      res.status(400).json({
        success: false,
        message: "Không thể chỉnh sửa món ăn trong đơn đặt trước đã thanh toán",
      });
      return;
    }
    ...
    try {
      ...
      if (tableId != null) {
        const [tableRows] = await conn.query<RowDataPacket[]>(
          `SELECT status FROM pos_tables WHERE table_id = ? LIMIT 1 FOR UPDATE`,
          [tableId],
        );
        if (tableRows[0] && String(tableRows[0].status) === "reserved") {
          await conn.rollback();
          res.status(400).json({
            success: false,
            message: "Bàn đang ở trạng thái đặt trước và chưa check-in. Vui lòng quét mã QR hoặc check-in trước khi thanh toán.",
          });
          return;
        }
      }
      ...
      if (pm === "cash") {
        ...
        if (effectiveBookingId != null && Number.isFinite(effectiveBookingId)) {
          const [activeResRows] = await conn.query<RowDataPacket[]>(
            `SELECT reservation_id FROM booking_table_reservations
             WHERE booking_id = ?
               AND status IN ('active', 'checked_in')`,
            [effectiveBookingId],
          );

          if (activeResRows.length === 0) {
            await conn.query(
            ...
            if (bUserRows[0]) {
              const bookingUserId = bUserRows[0].user_id ? Number(bUserRows[0].user_id) : null;
              const locId = Number(bUserRows[0].location_id);
              if (bookingUserId != null && Number.isFinite(bookingUserId)) {
                publishToUser(bookingUserId, {
                  type: "booking_checked_in",
                  booking_id: effectiveBookingId,
...
export const getTouristTicketToday = async (
  ...
  try {
    ...
    const [soldBooking] = await pool.query<RowDataPacket[]>(
      `SELECT bt.service_id, COUNT(*) AS sold_booking
       FROM booking_tickets bt
  ...
  try {
    ...
    for (const r of posPayRows) {
      ...
      const items: PosInvoiceItem[] = itemsRaw
        .map((it: any): PosInvoiceItem | null => {
          ...
          return {
            ...
            service_name: serviceName || "-",
            quantity: qty,
            used_quantity: qty, // POS offline mặc định đã soát
      ...
      posInvoices.push({
        ...
        owner_receivable: Number(r.owner_receivable || 0),
        check_in_date: toIso(r.payment_time), // POS mặc định sử dụng trong ngày mua
        items,
      });
    }
    ...
    const [bookingItemRows] = await pool.query<RowDataPacket[]>(
      `SELECT bt.booking_id,
              ...
              COUNT(*) AS quantity,
              SUM(CASE WHEN bt.status = 'used' THEN 1 ELSE 0 END) AS used_quantity
       FROM booking_tickets bt
       JOIN bookings b ON b.booking_id = bt.booking_id
       JOIN users u ON u.user_id = b.user_id
       JOIN services s ON s.service_id = bt.service_id
    ...
    for (const r of bookingItemRows) {
      ...
      const qty = Number(r.quantity || 0);
      const usedQty = Number(r.used_quantity || 0);
      ...
      const inv = bookingById.get(bookingId) ?? {
        ...
        owner_receivable: r.owner_receivable != null ? Number(r.owner_receivable) : 0,
        check_in_date: r.check_in_date ? toIso(r.check_in_date) : null,
        voucher_code: r.voucher_code ? String(r.voucher_code) : null,
      ...
      inv.commission_amount = r.commission_amount != null ? Number(r.commission_amount) : inv.commission_amount;
      inv.owner_receivable = r.owner_receivable != null ? Number(r.owner_receivable) : inv.owner_receivable;
      inv.items.push({
        service_id: Number(r.service_id),
    ...
    const bookingInvoices = Array.from(bookingById.values()).map((inv) => ({
      ...inv,
      invoice_no: inv.payment_id ? (seqMap.get(Number(inv.payment_id)) || 1) : 1,
    }));
    ...
    // Đánh lại số hóa đơn để mỗi row có số duy nhất
    let invCounter = 0;
    for (const inv of invoices) {
      invCounter++;
      inv.invoice_no = invCounter;
    }
...
export const scanTouristTicket = async (
  ...
  try {
    ...
    try {
      ...
      // Detect Group QR scan code
      const groupMatch =
        code.match(/^GROUP_BOOKING:(\d+)$/i) ||
        code.match(/^SB-(\d+)-GROUP$/i) ||
        code.match(/^SB-(\d+)$/i);
      if (groupMatch) {
        const bookingId = Number(groupMatch[1]);
        ...
        const [btRows] = await conn.query<RowDataPacket[]>(
          `SELECT bt.ticket_id, bt.ticket_code, bt.status, s.service_name, b.check_in_date,
           ...
           JOIN bookings b ON b.booking_id = bt.booking_id
           LEFT JOIN users u ON u.user_id = b.user_id
           WHERE bt.location_id = ? AND bt.booking_id = ?
           FOR UPDATE`,
          [locationId, bookingId],
        );
        ...
        if (!btRows || btRows.length === 0) {
          ...
          res.status(404).json({
            success: false,
            message: "Không tìm thấy nhóm vé hợp lệ",
          });
          return;
        }
        ...
        const ticketUseDateStr = btRows[0].check_in_date
          ? getLocalDateString(new Date(btRows[0].check_in_date))
          : null;
        if (ticketUseDateStr && ticketUseDateStr !== todayStr) {
          await conn.rollback();
          res.status(400).json({
            success: false,
            message: `Nhóm vé này chỉ có hiệu lực sử dụng vào ngày ${new Date(btRows[0].check_in_date).toLocaleDateString("vi-VN")}. Hôm nay không phải ngày sử dụng!`,
          });
          return;
        }

        const unusedTickets = btRows.filter((t) => String(t.status) === "unused");
        if (unusedTickets.length === 0) {
          await conn.rollback();
          res.status(400).json({
            success: false,
            message: `Tất cả vé thuộc nhóm vé #${bookingId} đã được sử dụng từ trước`,
          });
          return;
        }
        ...
        if (!isPaid) {
          ...
          res.status(400).json({
            success: false,
            message: "Nhóm vé chưa được thanh toán hoặc đặt chỗ chưa xác nhận",
          });
          return;
        }

        const ticketIds = unusedTickets.map((t) => t.ticket_id);
        await conn.query(
        ...
        res.json({
          ...
          data: {
            source: "booking",
            ticket_code: code,
            service_name: `Nhóm vé #${bookingId} (${unusedTickets.length} vé)`,
            contact_name: btRows[0]?.contact_name || null,
            contact_phone: btRows[0]?.contact_phone || null,
          },
        });
        return;
      }
      ...
      if (btRows[0]) {
        ...
        if (ticketUseDateStr && ticketUseDateStr !== todayStr) {
          ...
          res.status(400).json({
            success: false,
            message: `Vé này chỉ có hiệu lực sử dụng vào ngày ${new Date(btRows[0].check_in_date).toLocaleDateString("vi-VN")}. Hôm nay không phải ngày sử dụng!`,
          });
          return;
        }
...
export const payTouristPosTicketsBatch = async (
  ...
  try {
    ...
    try {
      ...
      for (const [serviceId, qty] of merged) {
        const svc = svcById.get(serviceId)!;
        const remainingToday = await getServiceRemainingQuantity(
          conn,
          serviceId,
          locationId,
          targetDate
        ...
        if (qty > remainingToday) {
          insufficient.push({
            service_id: serviceId,
            service_name: svc.service_name,
            total_today: svc.quantity,
            sold_today: svc.quantity - remainingToday,
            remaining_today: remainingToday,
            requested: qty,
          });
        }
      ...
      const issueTickets = async () => {
        const codes: string[] = [];
        const insertSql = posSupport.hasSoldBy
          ? `INSERT INTO pos_tickets (location_id, service_id, ticket_code, status, sold_by, used_at, used_by)
             VALUES (?, ?, ?, 'used', ?, CURRENT_TIMESTAMP, ?)`
          : `INSERT INTO pos_tickets (location_id, service_id, ticket_code, status, used_at, used_by)
             VALUES (?, ?, ?, 'used', CURRENT_TIMESTAMP, ?)`;
        for (const it of itemsSnap) {
          ...
          for (let i = 0; i < qty; i++) {
            ...
            await conn.query(
              insertSql,
              posSupport.hasSoldBy
                ? [locationId, serviceId, code, auth.userId, auth.userId]
                : [locationId, serviceId, code, auth.userId],
            );
          }
        }

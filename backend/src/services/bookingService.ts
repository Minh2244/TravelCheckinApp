...
import { generateSecureQrPayload } from "./bookingQrService";
...
export const getServiceRemainingQuantity = async (
  ...
  const [svcRows] = await connection.query<RowDataPacket[]>(
    `SELECT quantity FROM services WHERE service_id = ? AND location_id = ? LIMIT 1`,
    [serviceId, locationId]
  );
  if (!svcRows || svcRows.length === 0) return 0;
  const maxCapacity = Number(svcRows[0].quantity || 0);
  ...
  const [posRows] = await connection.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS cnt
     FROM pos_tickets
     WHERE service_id = ? AND DATE(sold_at) = ? AND status <> 'void'`,
    [serviceId, targetDate]
  );
  const posSold = Number(posRows?.[0]?.cnt || 0);

  return Math.max(0, maxCapacity - (onlineSold + posSold));
};
...
interface VoucherRow extends RowDataPacket {
  ...
  target_group: string;
  loyalty_min_spend: string | number | null;
}
...
const validateVoucherForBooking = async (params: {
    ...
    const [rows] = await connection.query<VoucherRow[]>(
      `SELECT
       ...
       WHERE v.code = ?
         ...
         AND v.end_date >= NOW()
       FOR UPDATE`,
      [voucherCode, locationId, locationId, locationId, locationId, locationId, locationId],
    );
    voucherRows = rows;
  } catch (err: any) {
  ...
  if (voucher.target_group === 'loyal') {
    const minSpend = toNumber(voucher.loyalty_min_spend);
    if (minSpend > 0) {
  ...
  try {
    ...
    if (svc.service_type === "ticket") {
      ...
      for (const it of issuance) {
        ...
        const remaining = await getServiceRemainingQuantity(
          connection,
          Number(it.serviceId),
          locationId,
          targetDate
        ...
        if (remaining < it.quantity) {
          throw new BookingError(
            `Số lượng vé cho ngày ${targetDate} không đủ (Còn lại: ${remaining})`,
            409
          );
        }
      }
    ...
    if (voucherCode && String(voucherCode).trim() !== "") {
      ...
      campaignName = calc.campaignName || null;
    }
    ...
    const effectiveNotes = defaultTableNote ?? notes;
    ...
    if (normalizedVoucherCode) {
      const vcLabel = campaignName
        ? `VC: ${campaignName}`
        : `VC: ${normalizedVoucherCode}`;
      storedNotes = `[${vcLabel}] ${storedNotes || ""}`.trim();
    }
    ...
    if (svc.service_type === "table" && preorderItems.length > 0) {
      ...
      const orderDiscount = Math.min(discountAmount, preorderSubtotal);
      const orderFinal = roundMoney(preorderSubtotal - orderDiscount);
...
export const createBookingBatch = async (
  ...
  try {
    ...
    // Validate voucher once for the entire batch
    let batchDiscountAmount = 0;
    let batchFinalAmount = totalAmount;
    let normalizedVoucherCode: string | null = null;
    let appliedVoucherId: number | null = null;
    let campaignName: string | null = null;
    ...
    if (voucherCode && String(voucherCode).trim() !== "") {
      ...
      const calc = await validateVoucherForBooking({
        ...
        voucherCode: normalizedVoucherCode,
        locationId,
        locationType: String((svcRows as any)?.[0]?.location_type || "all") as VoucherLocationScope,
        serviceType: "room",
        totalAmount,
        consumeUsage: true,
      });

      batchDiscountAmount = calc.discountAmount;
      batchFinalAmount = calc.finalAmount;
      appliedVoucherId = calc.voucherId;
      campaignName = calc.campaignName || null;
    }
    ...
    // Precompute voucher notes prefix
    let voucherNotePrefix = "";
    if (normalizedVoucherCode) {
      const vcLabel = campaignName
        ? `VC: ${campaignName}`
        : `VC: ${normalizedVoucherCode}`;
      voucherNotePrefix = `[${vcLabel}] `;
    }

    const bookingIds: number[] = [];
    let remainingDiscount = batchDiscountAmount;
    for (const row of svcRows) {
      const unitPrice = toNumber(row.price);
      const perTotal = roundMoney(unitPrice * roomStayHours);

      // Distribute voucher discount proportionally across rooms
      let perDiscount = 0;
      if (batchDiscountAmount > 0 && totalAmount > 0) {
        if (bookingIds.length === svcRows.length - 1) {
          // Last room gets remaining discount to avoid rounding errors
          perDiscount = remainingDiscount;
        } else {
          perDiscount = roundMoney((perTotal / totalAmount) * batchDiscountAmount);
          remainingDiscount = roundMoney(remainingDiscount - perDiscount);
        }
      }
      ...
      if (voucherNotePrefix) {
        storedNotes = `${voucherNotePrefix}${storedNotes || ""}`.trim();
      }
...
export const confirmTicketBankTransfer = async (params: {
  ...
  try {
    ...
    const [bRows] = await connection.query<RowDataPacket[]>(
      `SELECT b.booking_id, b.user_id, b.location_id, b.service_id, b.quantity, b.check_in_date,
    ...
    // Lock services and check dynamic stock based on check_in_date
    const targetDate = booking.check_in_date
      ? new Date(booking.check_in_date).toISOString().slice(0, 10)
    ...
    for (const it of items) {
      ...
      if (remaining < it.quantity) {
        throw new BookingError(`Số lượng vé cho ngày ${targetDate} không đủ (Còn lại: ${remaining})`, 409);
      }
    }

    // NO static stock decrement for ticket bookings to prevent daily limit leak!

    // Issue tickets if not already issued
    if (!Array.isArray(existingTickets) || existingTickets.length === 0) {
      const generateSecureHash = (length: number = 6): string => {
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT
     ...
     WHERE b.user_id = ?
       ...
       AND r.status IN ('active','cancelled','checked_in','released')
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      ...
      const secureHash = crypto
        ...
        .substring(0, 6)
        .toUpperCase();
      const secureCode = `DI-${bookingId}-${secureHash}`;
      const qrPayload = generateSecureQrPayload(bookingId, Number(row.location_id), "table");
...
export const listMyRoomBookings = async (params: {
  userId: number;
  locationId?: number | null;
}): Promise<RoomBookingReservationSummary[]> => {
  const { userId, locationId } = params;
  if (!Number.isFinite(userId) || userId <= 0) {
    throw new BookingError("Chưa đăng nhập", 401);
  }
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT
     ...
     JOIN locations l ON l.location_id = b.location_id
     JOIN services s ON s.service_id = b.service_id
     LEFT JOIN hotel_stays hs ON hs.booking_id = b.booking_id
  ...
  const now = new Date();
  const byBooking = new Map<number, RoomBookingReservationSummary>();

  for (const row of rows) {
    const bookingId = Number(row.booking_id);
    if (!Number.isFinite(bookingId)) continue;

    const checkInDate = row.check_in_date ? formatMysqlDateTime(new Date(row.check_in_date)) : "";
    const checkOutDate = row.check_out_date ? formatMysqlDateTime(new Date(row.check_out_date)) : null;

    let nightCount = 1;
    if (row.check_in_date && row.check_out_date) {
      const diffTime = Math.abs(new Date(row.check_out_date).getTime() - new Date(row.check_in_date).getTime());
      nightCount = Math.max(1, Math.ceil(diffTime / (1000 * 60 * 60 * 24)));
    }
    ...
    if (!existing) {
      ...
      const secureHash = crypto
        ...
        .substring(0, 6)
        .toUpperCase();
      const secureCode = `RS-${bookingId}-${secureHash}`;
      const qrPayload = generateSecureQrPayload(bookingId, Number(row.location_id), "room");
    ...
    const roomNumber = row.room_number ? String(row.room_number) : null;
    if (roomNumber && !item.roomNames.includes(roomNumber)) {
      item.roomNames.push(roomNumber);
    }
  }

  for (const item of byBooking.values()) {
    if (item.roomNames.length === 0) {
      item.roomNames.push("Đang sắp xếp");
    }
  }
...
export const cancelMyTableBooking = async (params: {
  ...
  try {
    ...
    // Kiểm tra xem khách đã thanh toán chưa
    const [paidCheck] = await conn.query<RowDataPacket[]>(
      `SELECT payment_id FROM payments 
       WHERE booking_id = ? AND status = 'completed' LIMIT 1`,
      [bookingId]
    );
    if (paidCheck[0]) {
      throw new BookingError(
...
export const cancelMyBooking = async (params: {
  userId: number;
  bookingId: number;
}): Promise<{ bookingId: number }> => {
  const { userId, bookingId } = params;
  ...
  try {
    ...
    // 1) Lấy thông tin booking để kiểm tra quyền sở hữu và dịch vụ
    const [bookingRows] = await conn.query<RowDataPacket[]>(
      `SELECT b.booking_id, b.user_id, b.location_id, b.status, b.pos_order_id, 
              s.service_type, l.owner_id, l.location_type
       FROM bookings b
       JOIN services s ON s.service_id = b.service_id
       JOIN locations l ON l.location_id = b.location_id
       WHERE b.booking_id = ?
    ...
    const booking = Array.isArray(bookingRows) ? bookingRows[0] : null;
    if (!booking) {
      throw new BookingError("Không tìm thấy đơn đặt chỗ này", 404);
    }
    if (Number(booking.user_id) !== userId) {
    ...
    // 3) Giải phóng các tài nguyên liên quan tùy theo loại dịch vụ
    if (serviceType === "table") {
      ...
      // Giải phóng bàn trống trong pos_tables
      const [tableRows] = await conn.query<RowDataPacket[]>(
        `SELECT table_id
         FROM booking_table_reservations
         WHERE booking_id = ?`,
      ...
      // Hủy đơn preorder đi kèm bàn ăn nếu đang mở
      if (booking.pos_order_id != null) {
        await conn.query(
          `UPDATE pos_orders
           SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP
    ...
    } else if (serviceType === "room" && (locationType === "hotel" || locationType === "resort")) {
      // Hủy trạng thái lưu trú khách sạn
      await conn.query(
        `UPDATE hotel_stays
      ...
      // Lấy room_id để giải phóng trạng thái phòng
      const [stayRows] = await conn.query<RowDataPacket[]>(
        `SELECT room_id FROM hotel_stays WHERE booking_id = ? LIMIT 1`,
        [bookingId]
      );
      if (stayRows[0]) {
      ...
      // Hủy các vé du lịch chưa soát
      await conn.query(
        `UPDATE booking_tickets
         SET status = 'void'
         WHERE booking_id = ? AND status = 'unused'`,
        [bookingId]
      );
    }
    ...
    // 4) Phát thông báo Realtime qua hệ thống để cập nhật màn hình vận hành (Sơ đồ bàn/phòng) của Owner
    if (Number.isFinite(ownerId)) {
      if (serviceType === "room") {
        void publishHotelUpdatedForLocation({
          locationId,

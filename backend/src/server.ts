...
const startServer = async () => {
  try {
    ...
    try {
      await pool.query(
        `UPDATE locations 
         SET auto_cancel_hotel_minutes = 60 
         WHERE location_type IN ('hotel', 'resort') 
           AND (auto_cancel_hotel_minutes IS NULL OR auto_cancel_hotel_minutes = 4320)`
      );
    } catch (e) {
    ...
    const autoCancelAndExpireBookings = async () => {
          ...
          `UPDATE bookings b
           ...
           SET b.status = 'cancelled',
               ...
               b.notes = CONCAT(
                 COALESCE(b.notes, ''),
                 CASE WHEN b.notes IS NULL OR b.notes = '' THEN '' ELSE '\n' END,
                 CONCAT('[SYSTEM] Tự động hủy: trễ hơn ', COALESCE(l.auto_cancel_food_minutes, 60), ' phút')
               )
          ...
          `UPDATE bookings b
           ...
           SET b.status = 'cancelled',
               ...
               b.notes = CONCAT(
                 COALESCE(b.notes, ''),
                 CASE WHEN b.notes IS NULL OR b.notes = '' THEN '' ELSE '\n' END,
                 CONCAT('[SYSTEM] Tự động hủy: trễ hơn ', ROUND(COALESCE(l.auto_cancel_hotel_minutes, 60) / 60, 1), ' giờ')
               )
           WHERE b.status IN ('pending','confirmed')
          ...
          `UPDATE bookings b
           ...
           SET b.status = 'cancelled',
               ...
               b.notes = CONCAT(
                 COALESCE(b.notes, ''),
                 CASE WHEN b.notes IS NULL OR b.notes = '' THEN '' ELSE '\n' END,
                 CONCAT('[SYSTEM] Vé hết hạn sau ', ROUND(COALESCE(l.auto_cancel_ticket_minutes, 1440) / 1440, 1), ' ngày')
               )
           WHERE b.status IN ('pending','confirmed')
      ...
      try {
        // Tự động hết hạn vé tại thời điểm đóng cửa (check_out_date)
        const [rows] = await pool.query(
          `SELECT b.booking_id
           FROM bookings b
           JOIN services s ON s.service_id = b.service_id
      ...
      // Phát sự kiện Realtime cập nhật trạng thái về client qua SSE
      try {
        const rangeEnd = new Date();

        // 1) Phát sự kiện booking_expired cho khách hàng
        const [expiredUserRows] = await pool.query(
          `SELECT booking_id, user_id, location_id
           ...
           WHERE status = 'cancelled'
             ...
             AND notes LIKE '%[SYSTEM]%'
           ORDER BY cancelled_at ASC
           LIMIT 2000`,
          [rangeStart, rangeEnd],
        );

        for (const r of expiredUserRows as any[]) {
        ...
        // 2) Phát sự kiện pos_updated / pms_updated cho các Owners và Employees đang mở màn hình điều hành
        const [updatedLocationRows] = await pool.query(
          `SELECT DISTINCT b.location_id, l.owner_id
           FROM bookings b
           JOIN locations l ON l.location_id = b.location_id
           WHERE (b.status = 'cancelled' AND b.cancelled_by IS NULL AND b.cancelled_at > ? AND b.cancelled_at <= ?)
              OR (b.status = 'confirmed' AND b.notes LIKE '%[SYSTEM] Tự động xác nhận%' AND b.updated_at > ? AND b.updated_at <= ?)`,
          [rangeStart, rangeEnd, rangeStart, rangeEnd],
        );

        for (const r of updatedLocationRows as any[]) {
          const locId = Number(r.location_id);
          const ownerId = Number(r.owner_id);
          if (Number.isFinite(locId) && Number.isFinite(ownerId)) {
            const ids = new Set<number>();
            ids.add(ownerId);

            const [empRows] = await pool.query(
              `SELECT employee_id FROM employee_locations WHERE owner_id = ? AND location_id = ? AND status = 'active'`,
              [ownerId, locId],
            );
            for (const emp of empRows as any[]) {
              ids.add(Number(emp.employee_id));
            }

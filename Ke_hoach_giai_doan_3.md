...
### 4.2. Giải pháp kỹ thuật phòng ngừa & khắc phục lỗi Bản đồ / La bàn (MỚI BỔ SUNG)

Nhằm tránh các tình trạng giật lag bản đồ, mũi tên xoay bị đơ/rung, hoặc đường chỉ đường vẽ sai lệch, lập trình viên cần tuân thủ nghiêm ngặt các giải pháp kỹ thuật sau:
...
#### 3. Khắc phục vẽ đường chỉ đường (Polyline) bị sai lệch:
- **Đồng bộ định dạng hệ toạ độ**:
  - API OSRM trả về định dạng GeoJSON là `[longitude, latitude]` (Kinh độ trước, Vĩ độ sau).
  - Component `<Polyline>` của `react-native-maps` yêu cầu định dạng `{ latitude: number, longitude: number }` (Vĩ độ trước, Kinh độ sau).
  - Phải thực hiện chuyển đổi (map) tường minh trước khi đưa vào component Polyline. Sử dụng thư viện `@mapbox/polyline` để giải mã (decode) chuỗi OSRM geometry chính xác.
- **Ngăn chặn gọi trùng và nhảy đường chỉ đi (GPS Drift)**: 
  - Chỉ gọi API OSRM tính lại đường đi khi người dùng di chuyển thực tế vượt quá 15 mét (dùng `@turf/distance` để đo khoảng cách GPS mới so với GPS cũ).
  - Không gọi lại API OSRM liên tục mỗi khi GPS cập nhật sai số nhỏ trong phạm vi vài mét.

...
const LocationDetail = () => {
  ...
  const locationVouchers = useMemo(() => {
    return locationVoucherList
      .filter((v: any) => {
        const maxUses = Number(v.max_uses_per_user || 0);
        const usedCount = Number(v.user_used_count || 0);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const [serviceResp, favoriteResp, reviewResp, voucherResp, savedResp] =
          await Promise.all([
            ...
            userApi.getMySavedVouchers().catch(() => ({ success: false, data: [] })),
          ]);
        ...
        if (!cancelled) {
          ...
          setSavedVoucherIds(
            savedResp.success
              ? (savedResp.data || []).map((v: any) => v.voucher_id)
              : [],
          );
        }
      } catch {
  ...
  const handleClaimVoucher = async (voucherId: number) => {
    ...
    // Optimistic: giảm remaining ngay
    setLocationVoucherList((prev) =>
      prev.map((v: any) =>
        v.voucher_id === voucherId
          ? { ...v, remaining: Math.max(0, (v.remaining || 0) - 1) }
          : v,
      ),
    );
    try {
      ...
      setLocationVoucherList((prev) =>
        prev.map((v: any) =>
          v.voucher_id === voucherId
            ? { ...v, remaining: (v.remaining || 0) + 1 }
            : v,
        ),
      );
      flashAction("Không thể lưu voucher");
    } finally {
      setClaimingId(null);
    }
  };

...
const BookingPage = () => {
  ...
  const [fadeNotice, setFadeNotice] = useState<{ message: string } | null>(null);
  const [fadeOpacity, setFadeOpacity] = useState(1);

  useEffect(() => {
    const msg = sessionStorage.getItem("tc_booking_fade_message");
    if (msg) {
      setFadeNotice({ message: msg });
      sessionStorage.removeItem("tc_booking_fade_message");
    }
  }, []);
  ...
  const loadRealtimeTicketStocks = useCallback(async () => {
    if (!locationIdNum) return;
    try {
      const res = await locationApi.getTicketsRealtimeStock(locationIdNum);
      if (res?.success && Array.isArray(res.data)) {
        const stocks = res.data;
        setServices((prev) => {
          return prev.map((s) => {
            const found = stocks.find((x) => Number(x.service_id) === s.service_id);
            if (found) {
              return {
                ...s,
                quantity: Number(found.remaining_today),
              };
            }
            return s;
          });
      ...
      console.error("Lỗi cập nhật số lượng vé realtime:", err);
    }
  }, [locationIdNum]);
  ...
  >("day");
  const [hotelCustomDays, setHotelCustomDays] = useState<number | string>(1);
  ...
  const hotelComputedCheckout = useMemo(() => {
    if (!isHotelBooking || !form.checkInDate || hotelStayDays <= 0) return "";
  ...
  const selectedTicketItems = useMemo(() => {
    ...
    for (const s of ticketServices) {
      ...
      const qtyRaw = ticketQtyByServiceId[sid];
  ...
  useEffect(() => {
    ...
    // Giữ mặc định ban đầu của vé là 0 theo yêu cầu người dùng
  ...
  const applicableVouchers = useMemo(() => {
    if (isFoodLocation && !preorderEnabled) return [];
  ...
  const cancelTableReservation = async (bookingId: number, typeLabel = "đặt bàn") => {
    if (!Number.isFinite(bookingId) || bookingId <= 0) return;

    Modal.confirm({
      title: `Xác nhận hủy ${typeLabel}?`,
      content: (
        <div className="space-y-2 text-sm text-slate-600 mt-2">
          <p className="font-semibold text-red-600">
            ⚠️ Lưu ý quan trọng: Hủy đặt chỗ trước giờ sử dụng sẽ KHÔNG được hoàn tiền dưới bất kỳ hình thức nào.
          </p>
          <p>
            Bạn có chắc chắn muốn đồng ý hủy đặt chỗ và giải phóng chỗ lập tức không?
          </p>
        </div>
      ),
      okText: "Đồng ý hủy (Chấp nhận mất tiền)",
      okType: "danger",
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (!res.success) {
            setFormError(res.message || `Không thể hủy ${typeLabel}`);
            return;
          }

          Modal.success({
            title: "Hủy thành công",
            content: `Đã hủy ${typeLabel} và giải phóng tài nguyên thành công!`,
            centered: true,
          });
          ...
          // Xóa khỏi notices nếu là phòng khách sạn
          setHotelNotices((prev) => prev.filter((n) => Number(n.id) !== bookingId));
        } catch (error) {
          setFormError(getErrorMessage(error, `Không thể hủy ${typeLabel}`));
        } finally {
  ...
  const handleSubmit = async () => {
    ...
    if (isHotelBooking) {
      if (hotelStayPreset === "custom") {
        const customDaysNum = Number(hotelCustomDays);
        if (hotelCustomDays === "" || Number.isNaN(customDaysNum) || customDaysNum < 1) {
          setFormError("Số ngày ở ít nhất là 1 ngày");
          return;
        }
        if (customDaysNum > 90) {
          setFormError("Thời gian lưu trú tối đa là 3 tháng (90 ngày)");
          return;
        }
      }
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {isTableBooking ? (
                <button
                  type="button"
                  className="rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700 hover:bg-amber-100"
                  onClick={() => {
                    const target = Number.isFinite(Number(locationIdNum))
                      ? `/user/table-pass?locationId=${Number(locationIdNum)}`
                      : "/user/table-pass";
                    navigate(target);
                  }}
                >
              ...
              {isRoomBooking || isHotelBooking ? (
                <button
                  type="button"
                  className="rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm text-indigo-700 hover:bg-indigo-100"
                  onClick={() => {
                    const target = Number.isFinite(Number(locationIdNum))
                      ? `/user/room-pass?locationId=${Number(locationIdNum)}`
                      : "/user/room-pass";
                    navigate(target);
                  }}
                >
                      ...
                      {ticketServices.length === 0 ? (
                        ...
                        <div className="mt-2 space-y-2">
                          {ticketServices.map((s) => {
                            const sid = Number(s.service_id);
                            const qtyRaw = ticketQtyByServiceId[sid];
                            const qty = qtyRaw === undefined ? 0 : qtyRaw;
                            const imgs = parseImages(s.images);
                            const img = resolveBackendUrl(imgs[0]);
                            const maxVal = s.quantity != null ? Number(s.quantity) : 999999;
                            return (
                              <div
                                key={s.service_id}
                                className="flex items-center justify-between gap-3 rounded-xl border border-gray-100 px-3 py-2"
                              >
                                <div className="min-w-0 flex items-center gap-3">
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <div className="text-[11px] text-gray-600 flex flex-wrap items-center gap-1.5 mt-0.5">
                                      <span>
                                        {Number(s.price || 0).toLocaleString("vi-VN")}đ
                                      </span>
                                      <span className="text-gray-300">•</span>
                                      <span className="text-emerald-700 font-medium bg-emerald-50 px-1.5 py-0.5 rounded-full text-[10px]">
                                        Còn lại: {s.quantity != null ? Number(s.quantity) : 0} vé
                                      </span>
                                    </div>
                                  </div>
                                </div>
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (Number.isFinite(next) && next > maxVal) {
                    ...
                    {hotelStayPreset === "custom" ? (
                      <div className="mt-3">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setHotelCustomDays(e.target.value);
                            resetBookingResults();
                          }}
                        />
                        {hotelCustomDays !== "" && Number(hotelCustomDays) < 1 ? (
                          <div className="mt-1 text-xs text-red-500 font-medium">
                            Ngày ở ít nhất là 1 ngày
                          </div>
                        ) : null}
                        {Number(hotelCustomDays) > 90 ? (
                          <div className="mt-1 text-xs text-red-500 font-medium">
                            Ngày ở tối đa là 90 ngày (3 tháng)
                          </div>
                        ) : null}
                      </div>
                    ) : null}
              ...
              {isHotelBooking ? (
                <div className="rounded-2xl border border-gray-100 p-4">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    ...
                    {voucherDiscount > 0 ? (
                      <>
                        <span className="line-through text-gray-400">{selectedTotal.toLocaleString("vi-VN")}đ</span>
                        {" "}
                        <b className="text-emerald-700">{Math.max(0, selectedTotal - voucherDiscount).toLocaleString("vi-VN")}đ</b>
                      </>
                    ) : (

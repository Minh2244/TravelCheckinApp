  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      setSummary({
        ...
        commission_rate: Number(asRecord(data.summary).commission_rate || 0),
        commission_amount: Number(
        ...
        h
          .map((x: any) => {
            ...
            return {
              ...
              voucher_code:
                r.voucher_code == null ? null : String(r.voucher_code),
              discount_amount:
                r.discount_amount == null
                  ? undefined
                  : Number(r.discount_amount),
              final_amount:
                r.final_amount == null ? undefined : Number(r.final_amount),
            } as HistoryRow;
          })
      ...
      <div className="rounded-2xl border bg-white p-4">
              ...
              {roomsForRender.map((rm, idx) => (
                ...
                >
                  {(() => {
                    ...
                    const roomAmount = hasPrepaid
                      ? Number(rm.onsite_amount ?? rm.total_amount ?? 0)
        ...
        {roomsForRender.length > 0 ? null : (
          <div className="mt-4 flex items-end justify-between gap-4 border-t pt-3">
            ...
            <div className="text-lg font-bold">{formatMoney(getEffectiveInvoiceTotal(row))}</div>
          </div>
        )}
  ...
  const restaurantColumns = useMemo(
    () => [
      ...
      },
      {
        title: "Số tiền",
        dataIndex: "amount",
        render: (_v: number, row: HistoryRow) => <b>{formatMoney(getEffectiveInvoiceTotal(row))}</b>,
      },
      {
        title: "Hoa hồng",
        dataIndex: "commission_amount",
                                ...
                                <g>
                                  ...
                                  <g>
                                    <text
                                      ...
                                      fill="#fff"
                                    >
                                      {`Tổng: ${formatMoney(hp.totalValue)}  (HH: ${formatMoney(hp.commissionValue)})`}
                                    </text>
                                  </g>
                                </g>
                              );

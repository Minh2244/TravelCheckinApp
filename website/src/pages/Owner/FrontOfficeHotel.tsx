...
export default function FrontOfficeHotel(props: {
          ...
          <>
            {onsiteAmount > 0 && (
              <>
                ...
                <div className="text-right font-semibold">
                  {formatMoney(prepaidAmount + onsiteAmount)}
                </div>
              </>
            )}
  ...
  const load = useCallback(
    async (selectedFloor: string, silent = false) => {
      if (!silent) setLoading(true);
      try {
        const [rRes, bRes] = await Promise.all([
          ownerApi.getHotelRooms({
        ...
        const raw = (rRes?.data || []) as unknown[];
        const mapped: RoomRow[] = raw
          .map((item) => {
            ...
            return {
              ...
              booking_created_at:
                r.booking_created_at == null
                  ? null
                  : String(r.booking_created_at),
  ...
  const handleAcceptBooking = async (bookingId: number) => {
    ...
    try {
      ...
      if (res.success) {
        message.success("Đã chấp nhận đơn đặt phòng!");
        void load(floorRef.current, true);
      } else {
  ...
  const handleRejectBooking = async (bookingId: number) => {
    Modal.confirm({
      title: "Từ chối đơn đặt phòng này?",
      content: "Đơn đặt phòng sẽ bị hủy và giải phóng phòng lập tức. Hành động này không thể hoàn tác.",
      okText: "Từ chối",
      okType: "danger",
      cancelText: "Hủy bỏ",
      centered: true,
        ...
        Modal.success({
          ...
          content: (
            <div className="space-y-2 mt-3 text-sm text-slate-700">
              ...
              <div className="flex items-center gap-1.5">
                <span className="font-semibold text-slate-500 w-24">Phòng gán:</span>
                <span className="font-bold text-blue-700">{res.data?.roomNames?.join(", ") || "Chưa gán phòng"}</span>
              </div>
  ...
  const renderPendingPanel = () => {
    return (
      <div className="max-h-[55vh] overflow-y-auto pr-1 space-y-3 mt-3">
        {pendingBookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 rounded-2xl border border-dashed bg-slate-50/50 p-4 text-center">
            <span className="text-2xl">☕</span>
            <div className="mt-2 text-xs font-semibold text-slate-850">Không có đơn chờ duyệt</div>
            <div className="text-[10px] text-slate-500 mt-0.5">Khách đặt phòng online thành công sẽ xuất hiện ở đây để duyệt.</div>
          </div>
        ) : (
          pendingBookings.map((b: any) => {
            ...
            return (
              <div 
                key={bookingId}
                className="p-3 rounded-2xl border border-amber-200 bg-amber-50/20 shadow-sm space-y-2.5 transition-all hover:shadow text-left"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="text-xs font-bold text-slate-800">
                      Đơn phòng #RS-{bookingId}
                    </div>
                    <div className="text-[10px] text-slate-505 mt-0.5 font-medium truncate">
                      {b.contact_name} • {b.contact_phone}
                    </div>
                  </div>
                  <span className="bg-amber-100 text-amber-800 border border-amber-200 text-[9px] font-extrabold px-1.5 py-0.5 rounded-full uppercase animate-pulse shrink-0">
                    Chờ duyệt
                  </span>
                </div>

                <div className="text-[11px] text-slate-700 bg-white/75 p-2 rounded-xl border border-slate-100/70 space-y-1">
                  <div>
                    🏨 Loại phòng: <b className="text-blue-700 font-semibold">{b.service_name || "Phòng nghỉ"}</b>
                  </div>
                  <div>
                    📅 Nhận: <b>{b.check_in_date ? formatDateTimeVi(b.check_in_date) : "-"}</b>
                  </div>
                  <div>
                    📅 Trả: <b>{b.check_out_date ? formatDateTimeVi(b.check_out_date) : "-"}</b>
                  </div>
                  {b.notes && (
                    <div className="text-slate-500 italic mt-0.5 break-words">
                      Ghi chú: "{b.notes}"
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5 pt-0.5">
                  <Button
                    type="primary"
                    size="small"
                    className="flex-1 h-8 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-700 border-none"
  ...
  const renderQrPanel = () => {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-4 text-left mt-3">
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-2 pt-1">
            <Button
              type="primary"
              className="w-full rounded-xl h-10 font-bold text-xs bg-gradient-to-r from-blue-600 to-indigo-600 border-none shadow-md shadow-blue-500/10 flex items-center justify-center gap-2"
              onClick={() => {
            ...
            <Button
              type="default"
              className="w-full rounded-xl h-10 font-bold text-xs border-slate-200 hover:border-indigo-500 text-slate-700"
  ...
  const renderSidebar = () => (
    <Card size="small" className="border border-blue-100 shadow-sm">
      <div className="mb-3 flex flex-row items-center overflow-x-auto whitespace-nowrap gap-1.5 p-1 bg-slate-100/80 rounded-xl border border-slate-200/50 scrollbar-none">
        <button
          type="button"
          className={`flex-shrink-0 py-1.5 px-4 rounded-lg text-xs font-bold transition-all duration-300 whitespace-nowrap ${
            activeTab === "rooms"
        ...
        <button
          type="button"
          className={`flex-shrink-0 py-1.5 px-4 rounded-lg text-xs font-bold transition-all duration-300 whitespace-nowrap flex items-center justify-center gap-1.5 ${
            activeTab === "pending"
        ...
        >
          <span>Chờ duyệt</span>
          {pendingBookings.length > 0 && (
            <span className="bg-rose-500 text-white font-extrabold text-[9px] px-1.5 py-0.5 rounded-full animate-bounce">
                      ...
                      {roomsFiltered.map((r) => {
                        ...
                        const handleSelectForCheckin = () => {
                          if (r.status === "reserved") {
                            Modal.warning({
                              title: "Chính sách Check-in Bảo mật",
                              content: (
                                <div className="space-y-2 mt-2 text-sm text-slate-700 leading-relaxed">
                                  <p>Phòng này đã được đặt trước (<span className="text-amber-600 font-bold">ĐÃ ĐẶT</span>).</p>
                                  <p>Để đảm bảo an toàn và kích hoạt phòng theo đúng thứ tự, <b>tuyệt đối không cho phép mở phòng thủ công</b> tại quầy.</p>
                                  <p>Vui lòng yêu cầu khách hàng xuất trình vé hoặc mã QR check-in, sau đó nhấn nút <b>"Soát QR Check-in"</b> để quét hoặc nhập mã vé.</p>
                                </div>
                              ),
                              okText: "Đã hiểu",
                              centered: true,

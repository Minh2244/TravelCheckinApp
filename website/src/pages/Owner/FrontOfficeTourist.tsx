...
export default function FrontOfficeTourist(props: {
  ...
  const loadRealtimeTicketStocks = useCallback(async () => {
    ...
    try {
      const res = await locationApi.getTicketsRealtimeStock(locationId);
        ...
        setSummary((prev) => {
          ...
          return {
            ...
            services: prev.services.map((s) => {
              const found = stocks.find((x) => Number(x.service_id) === Number(s.service_id));
              if (found) {
                const newRemaining = Number(found.remaining_today);
                const newSold = Math.max(0, Number(s.total_today || 0) - newRemaining);
                return {
                  ...s,
  ...
  const scan = async (raw?: string) => {
    ...
    try {
      ...
      const contactName = data.contact_name ? String(data.contact_name) : null;
      const contactPhone = data.contact_phone ? String(data.contact_phone) : null;
      const ticketCode = data.ticket_code ? String(data.ticket_code) : c;
        ...
        content: (
          <div className="space-y-2 mt-3 text-sm text-slate-700">
            <div className="flex items-center gap-1.5">
              <span className="font-semibold text-slate-500 w-24">Hạng vé:</span>
              <span className="font-bold text-blue-700">{serviceName || "-"}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="font-semibold text-slate-500 w-24">Khách hàng:</span>
              <span className="font-bold text-slate-800">{contactName || "Ẩn danh"}</span>
            </div>
            <div className="flex items-center gap-1.5">
            ...
            <div className="pt-2 text-xs text-slate-400 italic">Mã bảo mật: {ticketCode}</div>
          </div>
        ),
        okText: "Đồng ý",
        centered: true,
  ...
  const serviceColumns: ColumnsType<any> = useMemo(
    () => [
      ...
      {
        title: "Online đã soát",
        dataIndex: "used_online",
        width: 120,
        render: (v: unknown) => Number(v || 0),
      },
      {
  ...
  const totalStats = useMemo(() => {
    ...
    const soldOnline = items.reduce((sum, x) => sum + Number((x as any).sold_online || 0), 0);
    const soldPos = items.reduce((sum, x) => sum + Number((x as any).sold_pos || 0), 0);
    const used = items.reduce((sum, x) => sum + Number(x.used_today || 0), 0);
    const usedOnline = items.reduce((sum, x) => sum + Number((x as any).used_online || 0), 0);
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xl font-bold text-gray-800">
                    {invoice.payment_id != null ? `#BT-${invoice.payment_id}` : "-"}
                  </div>
                </div>
              </div>

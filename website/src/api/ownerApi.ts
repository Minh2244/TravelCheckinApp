...
const ownerApi = {
  ...
  getVoucherUsageHistory: async (voucherId: number) => {
    const res = await axiosClient.get(`/owner/vouchers/${voucherId}/usage-history`);
    return res.data;
  },

  getVoucherStats: async () => {
    const res = await axiosClient.get("/owner/vouchers/stats");
    return res.data;
  },
  ...
  checkinSecureQr: async (locationId: number, qrData: string) => {
    const res = await axiosClient.post("/owner/front-office/checkins/secure-qr", {

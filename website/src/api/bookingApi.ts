...
const bookingApi = {
  ...
  getMyTablePass: async (locationId?: number) => {
    const response = await axiosClient.get<TableReservationsResponse>(
  ...
  getMyRoomPass: async (locationId?: number) => {
    const response = await axiosClient.get<RoomReservationsResponse>(
  ...
  cancelMyBooking: async (bookingId: number) => {

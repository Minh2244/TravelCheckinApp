...
## 6. Migration — Chuyển dữ liệu cũ sang mới

### 6.1 Script migration

```typescript
// backend/src/scripts/migrateImagesToDB.ts

async function migrateImages() {
  // 1. Seed image_categories
  // 2. Duyệt qua từng folder trong backend/uploads/
  //    avatars/ → category avatar_user/avatar_admin/avatar_owner (check DB để biết role)
  //    backgrounds/ → category user_background/admin_background
  //    locations/ → category location_image
  //    services/ → category service_image
  //    reviews/ → category review_image/review_reply_image
  //    checkins/ → category checkin_photo

  // 3. Với mỗi file:
  //    a. Đọc file buffer
  //    b. Resize + compress bằng sharp
  //    c. INSERT INTO images (category_id, original_name, mime_type, file_size, width, height, data, uploaded_by, uploaded_by_role)
  //    d. INSERT INTO entity_images (image_id, entity_type, entity_id, role, sort_order, is_primary)

  // 4. Mapping entity từ filename pattern:
  //    avatars/avatar-{userId}-*.jpg → entity_type='user', entity_id=userId, role='avatar'
  //    locations/location-{locationId}-*.jpg → entity_type='location', entity_id=locationId
  //    services/service-{serviceId}-*.jpg → entity_type='service', entity_id=serviceId
  //    reviews/review-{reviewId}-*.jpg → entity_type='review', entity_id=reviewId

  // 5. Cập nhật avatar_url/background_url trong bảng users → "/api/images/{newId}"
  // 6. Cập nhật first_image trong locations → "/api/images/{newId}"
}
```

### 6.2 Thứ tự migration

1. Tạo 3 bảng mới + seed image_categories
2. Chạy script migration (đọc file → nén → INSERT)
3. Cập nhật API responses (trả URL mới)
4. Test hiển ảnh trên website + mobile
5. Xóa thư mục `backend/uploads/`
6. Xóa `express.static("/uploads")` trong server.ts
7. Xóa `saveUploadedImageToUploads()` trong uploadImage.ts
...
| `backend/src/scripts/migrateImagesToDB.ts` | Script migration dữ liệu cũ |
